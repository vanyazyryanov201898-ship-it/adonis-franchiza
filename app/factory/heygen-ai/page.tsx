...
function ScriptTab() {
  ...
  const [platform, setPlatform] = useState(DIRECTION_DEFAULT_PLATFORMS["heygen-ai"][0]);
  ...
  const generate = () => {
    ...
    run(`HeyGen AI · ${activeTopic.slice(0,30)}`, async () => {
      const res = await fetch("/api/factory-script", {
        ...
        body: JSON.stringify({ direction: "heygen-ai", topic: activeTopic, trendContext: selectedTrend ?? undefined, platform: PLATFORMS.find(p => p.id === platform)?.label }),
      });
