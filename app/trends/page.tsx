...
import {
  ...
  ChevronRight, Play, Sparkles, Hash, ArrowUpRight,
  BarChart2,
} from "lucide-react";
...
const categories = ["Все", "Уход из найма", "Доходы бизнеса", "Франчайзинг", "Открытие бизнеса", "Мерч"];

const viralHooks = [
  { text: "Я работал 8 лет на дядю, а потом...", score: 96 },
  { text: "Показываю реальную выручку за месяц:", score: 94 },
  { text: "Вот сколько зарабатывает партнёр ADONIS:", score: 92 },
  { text: "Открыл ИП за 1 день — вот как:", score: 89 },
  { text: "Франшиза окупилась за 4 месяца:", score: 87 },
  { text: "Бизнес с нуля — мой путь до 300К:", score: 86 },
  { text: "Уволился с работы и не пожалел:", score: 85 },
  { text: "DTF-печать — золотая жила 2026:", score: 83 },
];
                  ...
                  <div className="space-y-2">
                    {selectedTopic.hooks.map((hook, i) => (
                      ...
                      >
                        ...
                        <span className="text-xs text-slate-400 group-hover:text-white transition-colors">{hook}</span>
                      </motion.div>
                    ))}
                  </div>
                </div>
                ...
                {viralHooks.map((hook, i) => (
                  <div key={i}>
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-xs text-slate-400 truncate pr-2">{hook.text}</p>
                    </div>
