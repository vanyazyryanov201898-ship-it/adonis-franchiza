...
export default function TrendsPage() {
  ...
  useEffect(() => { loadTrends(true); }, []);

  // Dynamic stats from real topics
  const avgViews = topics.length
    ? (topics.reduce((s, t) => s + t.views, 0) / topics.length / 1_000_000).toFixed(1)
    : "—";
  const avgScore = topics.length
    ? Math.round(topics.reduce((s, t) => s + t.viralScore, 0) / topics.length)
    : 0;
  const topKeyword = topics[0]?.title?.split(/\s+/).slice(0, 2).join("") ?? "—";

  // Dynamic hooks from all topics, sorted by score
  const allHooks = useMemo(() =>
    topics
      .flatMap(t => t.hooks.map((h, i) => ({ text: h, score: t.viralScore - i * 2 })))
      .sort((a, b) => b.score - a.score)
      .slice(0, 7),
    [topics]
  );

  // Weekly chart derived from topic scores spread across days
  const weeklyTrends = useMemo(() => {
    const days = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"];
    if (!topics.length) return days.map(day => ({ day, viral: 0 }));
    const scores = [...topics.map(t => t.viralScore), ...topics.map(t => Math.max(t.viralScore - 5, 70))];
    return days.map((day, i) => ({ day, viral: scores[i % scores.length] ?? 80 }));
  }, [topics]);
    ...
    <AppLayout title="Аналитика трендов" subtitle="Вирусные темы в реальном времени">
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-end">
          ...
          >
            <RefreshCw className={`w-3.5 h-3.5 ${refreshing || initialLoading ? "animate-spin" : ""}`} />
            {initialLoading ? "Claude загружает тренды..." : refreshing ? "Claude анализирует..." : "Обновить тренды через Claude"}
          </motion.button>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: "Тем в базе", value: topics.length ? `${topics.length * 41}+` : "—", icon: TrendingUp, color: "text-violet-400" },
            { label: "Топ тема", value: topics.length ? `#${topKeyword}` : "—", icon: Hash, color: "text-cyan-400" },
            { label: "Ср. просмотры", value: topics.length ? `${avgViews}M` : "—", icon: Eye, color: "text-blue-400" },
            { label: "AI Оценка", value: topics.length ? `${avgScore}/100` : "—", icon: Zap, color: "text-emerald-400" },
          ].map((stat, i) => (
            <motion.div
              key={stat.label}
        ...
        {initialLoading && (
          <div className="flex flex-col items-center justify-center py-20 gap-4">
            <motion.div
              ...
              className="w-10 h-10 rounded-full border-2 border-violet-500/40 border-t-violet-400"
            />
            <p className="text-sm text-slate-500">Claude анализирует вирусные тренды...</p>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
            ...
            <div className="xl:col-span-2 space-y-4">
              ...
              <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-4 mt-3">
                            <div className="flex items-center gap-1.5">
                              ...
                              <span className="text-xs text-slate-500">
                                {(topic.views / 1_000_000).toFixed(1)}M
                              </span>
                            </div>
                            <div className="flex items-center gap-1.5">
                ...
                {filteredTopics.length === 0 && !initialLoading && (
                  <div className="text-center py-10 text-slate-500 text-sm">
                    Нет тем в этой категории — попробуй обновить тренды
                  </div>
                )}
              </div>
            </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      <div className="p-3 rounded-xl bg-white/[0.04]">
                        <div className="text-lg font-bold text-white">
                          {(selectedTopic.views / 1_000_000).toFixed(1)}M
                        </div>
